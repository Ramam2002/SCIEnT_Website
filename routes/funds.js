...
router.get('/getFundsForm', csrfProtection, function(req, res, next) {
    res.render('funds', { csrfToken: req.csrfToken() });
});

/* to handle form submission for facilities */
router.post('/applyForFunds', function(req, res, next) {
    ...
    Donations.sync({ force: false }).then(function() {
        ...
        var payload = {
          ...
          'redirect_url': hostname + '/fundsSuccess',
          'send_email': true,
          'webhook': '',
          'send_sms': true,
        ...
        request(options, function(error, response, body){
          ...
          if(!error && response.statusCode == 201) {
            ...
            console.log('\nRecord inserted successfully into Funds table');
...
router.get('/fundsSuccess', function(req, res, next) {
...
router.post('/admin/getFundsDetails', function(req, res, next) {
  Funds.findOne({where: {id: req.body.fundsId}})
  .then( function(fundsRecord) {
    console.log(fundsRecord);
    res.send(JSON.stringify(fundsRecord));
  }).catch(function(err) {
    console.log(err);
  });
});
